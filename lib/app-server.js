...
AppServer.prototype.getConfigService = function () {
    var configService;
    if (this.config.db && this.config.deployment) {
        configService = ConfigService.create(db.connection.getDb(), {collection: 'schemaInfo'});
    }
...
AppServer.prototype.checkSchema = function () {
    ...
    return Promise.fnCall(
        function () {
            return self.getConfigService();
        })
        .then(function (configService) {
            return self.serviceContainer.checkSchemaVersions(configService);
        })
        .then(function () {
...
AppServer.prototype.isEmptyDB = function () {
    var deferred = Promise.defer();
    if (this.config.db && this.config.deployment) {
        db.connection.getDb().collectionNames(function (error, names) {
            if (error) {
...
AppServer.prototype.initSchema = function () {
    var configService, schemaInfo, self = this;
    ...
    return Promise.fnCall(
        ...
        .then(function (service) {
            ...
            return self.serviceContainer.getSchemaInfo(configService);
        })
        .then(function (info) {
            schemaInfo = info;
            return self.serviceContainer.checkDbSchemaIsLower(schemaInfo);
        })
        .then(function () {
        ...
        .then(function () {
            return self.serviceContainer.updateSchemaInfo(configService, schemaInfo);
        })
        .catch(function (error) {
...
AppServer.prototype.upgradeSchema = function () {
    ...
    return Promise.fnCall(
        ...
        .then(function (upgradeRequired) {
            if (upgradeRequired) {
                return self.serviceContainer.prepareSchemaUpgrade(schemaInfo)
                    .then(function () {
                        return self.serviceContainer.upgradeSchema(schemaInfo);
                    })
                    .then(function () {
                        return self.serviceContainer.onSchemaUpgraded(schemaInfo);
                    });
            }
        })
        .then(function () {
...
AppServer.prototype.initialize = function (customServices, maintenanceMode) {
    ...
    return this.dbConnect().
        then(function () {
            return self.isEmptyDB();
         })
        .then(function (isEmptyDB) {
            self.databaseIsEmpty = isEmptyDB;
            self.checkStartupAbort();
        ...
        .then(function () {
            if (!maintenanceMode) {
                ...
                return serviceContainer.checkSchemaVersions(configService);
            }
        })
        .then(function () {
            self.checkStartupAbort();
